from PrincyRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from PrincyRobot import DRAGONS
...
async def Princy_is_admin(chat_id: int):
    status = False
    Princy = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if Princy.id == user.id:
            status = True
            break
    return status
