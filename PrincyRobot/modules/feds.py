...
import PrincyRobot.modules.sql.feds_sql as sql
from PrincyRobot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from PrincyRobot.modules.disable import DisableAbleCommandHandler
from PrincyRobot.modules.helper_funcs.alternate import send_message
from PrincyRobot.modules.helper_funcs.chat_status import is_user_admin
from PrincyRobot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from PrincyRobot.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Princy_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Princy_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Princy_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Princy_fbanned_users.csv",
